...
public class Variant<T> {
     ...
     */
    public Variant(final T o) throws IllegalArgumentException {
        if (null == o) {
            throw new IllegalArgumentException(localize("Can't wrap Null in a Variant"));
        }
        type = o.getClass();
        try {
            final String[] ss = Marshalling.getDBusType(o.getClass(), true);
            if (ss.length != 1) {
                throw new IllegalArgumentException(localize("Can't wrap a multi-valued type in a Variant: ") + type);
            }
            this.sig = ss[0];
     ...
     */
    public Variant(final T o, final Type type) throws IllegalArgumentException {
        if (null == o) {
            throw new IllegalArgumentException(localize("Can't wrap Null in a Variant"));
        }
        this.type = type;
        try {
            final String[] ss = Marshalling.getDBusType(type);
            if (ss.length != 1) {
                throw new IllegalArgumentException(localize("Can't wrap a multi-valued type in a Variant: ") + type);
            }
            this.sig = ss[0];
     ...
     * @throws IllegalArugmentException If you try and wrap Null or an object which cannot be sent over DBus.
     */
    public Variant(final T o, final String sig) throws IllegalArgumentException {
        if (null == o) {
            throw new IllegalArgumentException(localize("Can't wrap Null in a Variant"));
        }
        this.sig = sig;
