...
public class DBusMatchRule {
    ...
    public DBusMatchRule(final String type, final String iface, final String member) {
        this.type = type;
        this.iface = iface;
        this.member = member;
    }

    public DBusMatchRule(final DBusExecutionException e) throws DBusException {
        this(e.getClass());
        member = null;
        type = "error";
    }

    public DBusMatchRule(final Message m) {
    ...
    public DBusMatchRule(final Class<? extends DBusInterface> c, final String method) throws DBusException {
        this(c);
        member = method;
        type = "method_call";
    }

    public DBusMatchRule(final Class<? extends Object> c, final String source, final String object)
    ...
    public DBusMatchRule(final Class<? extends Object> c) throws DBusException {
        if (DBusInterface.class.isAssignableFrom(c)) {
            ...
            if (!iface.matches(".*\\..*")) {
                throw new DBusException(localize("DBusInterfaces must be defined in a package."));
            }
            member = null;
            type = null;
            ...
            throw new DBusException(localize("Invalid type for match rule: ") + c);
        }
    }
